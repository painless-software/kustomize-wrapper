...
def version():
    ...
    kustomize_version = run_piped_commands(
        [f"{binarypath('kustomize')} version"]).stdout.strip()
    kubeval_version = run_piped_commands(
        [f"{binarypath('kubeval')} --version"]).stdout.strip()
